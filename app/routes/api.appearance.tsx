...
export const action = async ({ request }: ActionArgs) => {
  ...
  if (!actions.includes(_action)) return json({ ok: 0, reason: 'invalid action' })

  switch (_action) {
    case 'changeTheme':
      if (!themes.includes(theme)) return json({ ok: 0, reason: 'invalid theme' })
      return json({ ok: 1 }, { headers: { "Set-Cookie": await themeCookie.serialize(theme) } })
    case 'changeView':
      if (!views.includes(view)) return json({ ok: 0, reason: 'invalid view' })
