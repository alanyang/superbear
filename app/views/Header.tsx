...
import { useAppearanceState, useAppearanceStore, useUIState } from "~/utils/store";
...
export default ({ user, showSwither }) => {
  ...
  useEffect(() => setTransition(logoutClient.state), [logoutClient])
...
const TransitionPanel = () => {
  const transition = useUIState(s => s.transition)
