...
import { useAppearanceStore, useUIStore } from "~/utils/store";
...
export default ({ user, showSwither }) => {
  ...
  const ui = useUIStore()
  useEffect(() => { ui.transition = logoutClient.state }, [logoutClient])
...
const TransitionPanel = () => {
  const ui = useUIStore()
  const position = ui.transition === 'idle' ? '-right-80' : 'right-3'
  return (
    <div className={`fixed top-3 ${position} font-light ease-in-out duration-500`}>
      <div className="flex gap-3 shadow-lg rounded bg-blue-600/90 justify-center text-white p-4">
        ...
        <label className="capitalize">{ui.transition}...</label>
      </div>
    </div>
  )
}
...
const ViewSwither = () => {
  ...
  const gridBorder = appearance.view === 'list' ? 'border-slate-500' : 'border-slate-50'
  const listBorder = appearance.view === 'list' ? 'border-slate-500' : 'border-slate-50'

  return (
    <div className="pr-5">
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"
        onClick={event => appearance.view = "grid"}
      ...
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"
        onClick={event => appearance.view = "list"}
...
const ThemeSwither = () => {
  const appearance = useAppearanceStore()
  const pos = appearance.theme === 'light' ? 'left-1' : 'left-2/4'
  const toggle = () => {
    const newTheme = appearance.theme === "light" ? "dark" : "light";
    appearance.changeTheme(newTheme)
  }
  return (
